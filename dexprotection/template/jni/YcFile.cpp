...
YcFile::~YcFile() {
    ...
    if(NULL != globalEnv){
        globalEnv->DeleteGlobalRef(mYcFormat.indexHashMap);
    }
}

YcFile::YcFile(JNIEnv * env)
{
    mFilePath = NULL;
    globalEnv = env;
}
...
// 解析Yc文件。
bool YcFile::parse(JNIEnv* env, unsigned char* ycData, size_t dataSize) {
    MY_LOG_INFO("marshal_parseYC ==>");
    ...
    if (0 != separatorDataSize) {
       ...
       //创建索引map
        ...
        jmethodID mid_hashMap = env->GetMethodID(cls_hashMap, "<init>", "()V");

        mYcFormat.indexHashMap = env->NewGlobalRef(env->NewObject(cls_hashMap, mid_hashMap));
        ...
        for (int i = 0; i < separatorDataSize; i++) {
            ...
            mYcFormat.separatorDatas[i]->methodIndexForDexFile = ToUInt(p, dataSize);
            p += sizeof(mYcFormat.separatorDatas[i]->methodIndexForDexFile);
            MY_LOG_INFO("%d. methodIndexForDexFile:%u", i, mYcFormat.separatorDatas[i]->methodIndexForDexFile);

            setHashMapValue(env, mYcFormat.indexHashMap,
                    mYcFormat.separatorDatas[i]->methodIndexForDexFile,
                    mYcFormat.separatorDatas[i]->methodIndex);
            int zzzIndex = getHashMapValue(env, mYcFormat.indexHashMap, mYcFormat.separatorDatas[i]->methodIndexForDexFile);
            MY_LOG_INFO("%d. zzzIndex:%u", i, zzzIndex);
            ...
            for(int m = 0; m<mYcFormat.separatorDatas[i]->instSize;m++){
                MY_LOG_INFO("marshal_(%d,%d)separatorDatas[%d]->insts[%d]:%d",i,mYcFormat.separatorDatas[i]->methodIndexForDexFile, i,m, (mYcFormat.separatorDatas[i]->insts[m])&0xff);
            }
            p += instByteSize;
        }
    }
...
const SeparatorData* YcFile::getSeperatorByMethodIndex(JNIEnv * env, int methodIndexForDex)
{
    const SeparatorData * ret = NULL;
    int seperatorIndex = getHashMapValue( env, mYcFormat.indexHashMap, methodIndexForDex);
    MY_LOG_INFO("marshal_getSeperatorByMethodIndex seperatorIndex:%d", seperatorIndex);
    ret = GetSeparatorData(seperatorIndex);
    return ret;
}
...
YcFormat::YcFormat()
    : methods(NULL), separatorDatas(NULL), indexHashMap(NULL)
{
